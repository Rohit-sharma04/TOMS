...
export const Challan = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            console.log(option)
            let res = await axios.post("api/user/challan", { option });
    ...
    const makePayment = async(challanId)=>{
      try {
        console.log("challan",challanId)
        console.log("make payment")
        const res=await axios.post('api/stripe/create-checkout-session',{ challanId });
                ...
                <table className="w-full text-sm text-left rtl:textRight text-gray-500 dark:text-gray-400">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            {option === 'Pending' &&<th scope="col" className="px-6 py-3">
                               Action
                            </th>}
                        ...
                        {challan && challan.map((challan) => (
                            <React.Fragment key={challan._id}>
                                <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" onClick={() => toggleRow(challan._id)}>
                                    ...
                                    {option==='Pending'&&<td className="px-6 py-4">
                                        <button className="font-medium text-blue-600 dark:text-blue-500 hover:underline" onClick={()=>makePayment(challan._id)}>PAY</button>
                                    </td>}
                                </tr>
