...
import MonthlyChallanChart from "../Charts/MonthlyChallanChart";


export const Dashboard = () => {
  const [challanPaymentStatus, setChallanPaymentStatus] = useState([]);
  const [complaintsOverTimeData, setComplaintsOverTimeData] = useState([]);
  ...
  useEffect(() => {
    const fetchChallanData = async () => {
      try {
        const toatalChallanStatusData = await axios.get('api/admin/total-challan-status-data')
        setChallanPaymentStatus([toatalChallanStatusData.data.data.pendingCount, toatalChallanStatusData.data.data.completedCount]);

        const complaintsOverTimeResponse = await axios.get('api/admin/complaints-over-time');
        setComplaintsOverTimeData(complaintsOverTimeResponse.data.data);

        const complaintStatusResponse = await axios.get('api/admin/complaint-status-data');
        setComplaintStatusData(complaintStatusResponse.data.data);

        const monthlyChallanDataResponse = await axios.get('api/admin/monthly-challan-data');
        setMonthlyChallanData(monthlyChallanDataResponse.data.data)
  ...
  return (
    <>
      <main className="">
        ...
        <div className="p-5  sm:w-[70vw]">
          <ComplaintsOverTimeChart data={complaintsOverTimeData} />
        </div>

        <div className="p-5  sm:w-[70vw]">
          <MonthlyChallanChart data={monthlyChallanData} />
        </div>
